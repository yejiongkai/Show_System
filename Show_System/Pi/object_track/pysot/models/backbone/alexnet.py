...
class AlexNet_lite(nn.Module):
    configs = [3, 96, 256, 384, 256]  #  [3, 48, 128, 192, 128] 

    def __init__(self, width_mult=1):
        configs = list(map(lambda x: 3 if x == 3 else
                       int(x*width_mult), AlexNet_lite.configs))
        super(AlexNet_lite, self).__init__()
        self.layer1 = nn.Sequential(
            nn.Conv2d(configs[0], configs[1], kernel_size=11, stride=2),
            nn.BatchNorm2d(configs[1]),
            nn.MaxPool2d(kernel_size=3, stride=2),
...
def alexnet_lite(**kwargs):
    return AlexNet_lite(**kwargs)
